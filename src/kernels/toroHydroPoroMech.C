...
#include "toroHydroPoroMech.h"
#include "MooseMesh.h"

registerMooseObject("ToroApp", toroHydroPoroMech);

template <>
InputParameters
validParams<toroHydroPoroMech>()
{
...
toroHydroPoroMech::toroHydroPoroMech(const InputParameters & parameters)
...
Real
toroHydroPoroMech::computeQpResidual()
{
...
Real
toroHydroPoroMech::computeQpJacobian()
{
  return 0.0;
}
...
Real
toroHydroPoroMech::computeQpOffDiagJacobian(unsigned int jvar)
{
  for (unsigned int i = 0; i < _ndisp; ++i)
    if (jvar == _disp_var[i])
